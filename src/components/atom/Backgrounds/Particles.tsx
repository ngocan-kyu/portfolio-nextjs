...
const Particles = memo(function Particles({
  ...
  // Live props (để tránh rebind tick)
  ...
  // Tạo buffers khi count/colors đổi
  ...
  useLayoutEffect(() => {
    ...
    if (hoverRef.current) enablePointer();
    ...
    alphaParticles,
    cameraDistance,
    moveParticlesOnHover, // bind/unbind pointer listener on mount
  ]);

  // Nếu bật/tắt hover sau khi mount
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const onPointerMove = (e: PointerEvent) => {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        // Quan trọng: phải cho phép pointer events để nhận mouse move
        "absolute inset-0 w-full h-full overflow-hidden select-none pointer-events-auto",
        className
      )}
      aria-hidden="true"
    >
      {/* Overlay gradient (không chặn chuột) */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-black/10 via-black/0 to-black/40" />
      {/* Subtle glow */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_center,_rgba(255,255,255,0.08),_transparent_70%)] mix-blend-screen" />
    </div>
  );
});
