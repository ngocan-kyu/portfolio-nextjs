...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = data.map(item => document.querySelector(item.href) as HTMLElement);
      let current = "home";
  ...
  return (
    <div className="fixed top-5 right-0 left-0 px-0 sm:px-5 m-auto w-full sm:w-fit bg-transparent z-[9999]">
      <Dock className='items-end pb-3 rounded-full'>
        {data.map((item, idx) => (
          <Link href={item.href} key={idx} scroll={true}>
            <DockItem
              className={cn(
                "aspect-square rounded-full bg-gray-200 dark:bg-neutral-800",
                activeSection === item.title.toLowerCase() && "bg-gray-100 !border !border-primary-sky"
              )}
            >
